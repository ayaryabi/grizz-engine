...
class JSONWorkflowOrchestrator:
    """
    JSON-based workflow orchestrator - prototype for Redis hash approach
    Each tool reads from JSON, processes, writes back to JSON
    Demonstrates stateless tool architecture
    """
    
    def __init__(self, workflow_id: str = None):
        self.workflow_id = workflow_id or f"workflow_{int(time.time())}"
        self.json_file = f"{self.workflow_id}_state.json"
        self.workflow_state = {
    ...
    def _save_state(self):
        ...
        print(f"💾 Workflow state saved to {self.json_file}")
    ...
    def _start_step(self, step_name: str):
        """Mark start of a workflow step"""
        print(f"\n🔧 Starting Step: {step_name}")
        self._update_state({
            "current_step": step_name,
            "status": f"processing_{step_name}"
        })
        return time.time()
    
    def _complete_step(self, step_name: str, start_time: float, result: Any = None):
        """Mark completion of a workflow step"""
        duration = time.time() - start_time
        print(f"   ✅ Completed {step_name} in {duration:.2f}s")
        
        steps = self.workflow_state.get("steps_completed", [])
        steps.append(step_name)
        timing = self.workflow_state.get("timing", {})
        timing[f"{step_name}_time"] = duration
        ...
        if result is not None:
            updates[f"{step_name}_result"] = str(result)
        
        self._update_state(updates)
        return duration
    
    async def initialize_content(self, content: str):
        """Step 0: Initialize workflow with content"""
        start_time = self._start_step("initialize")
        
        # Store content in workflow state
        self._update_state({
            "content": content,
        ...
        self._complete_step("initialize", start_time)
        print(f"   📊 Content: {len(content)} chars, ~{len(content)//4} tokens")
    
    async def step_1_format_content(self):
        """Step 1: Format content using markdown formatter tool (stateless)"""
        start_time = self._start_step("format")
        
        # Tool reads from workflow state
        ...
        # Tool writes back to workflow state
        self._update_state({
            "formatted_content": result.formatted_content,
            "formatted_length": len(result.formatted_content)
        })
        
        duration = self._complete_step("format", start_time, "formatted successfully")
        print(f"   📝 Formatted to {len(result.formatted_content)} chars")
        return duration
    
    async def step_2_summarize_content(self):
        """Step 2: Summarize formatted content (stateless)"""
        start_time = self._start_step("summarize")
        ...
        self._update_state({
            ...
            "summary_length": len(result.summarized_content)
        })
        
        duration = self._complete_step("summarize", start_time, "summarized successfully")
        print(f"   📄 Summary: {len(result.summarized_content)} chars")
        return duration
    
    async def step_3_categorize_content(self):
        """Step 3: Categorize formatted content (stateless)"""
        start_time = self._start_step("categorize")
        ...
        self._update_state({
            ...
            "category_properties": result.properties
        })
        
        duration = self._complete_step("categorize", start_time, f"categorized as {result.category}")
        print(f"   🏷️  Category: {result.category}")
        return duration
    
    async def step_4_save_content(self):
        """Step 4: Save content to memory database (stateless)"""
        start_time = self._start_step("save")
        ...
        input_data = SaveMemoryInput(
            ...
            title=title,
            content=formatted_content,
            properties=state.get("category_properties", {}),
            category=category
        )
        ...
        duration = self._complete_step("save", start_time, f"saved with ID {result.id}")
        print(f"   💾 Saved with ID: {result.id}")
        return duration
    
    async def run_full_workflow(self, content: str):
        """Run the complete workflow: Initialize → Format → Summarize → Categorize → Save"""
        
        print(f"🚀 Starting JSON Workflow Orchestrator")
        print(f"📋 Workflow ID: {self.workflow_id}")
        print(f"📄 JSON State File: {self.json_file}")
        print("=" * 80)
        
        overall_start = time.time()
        
        try:
            # Run all workflow steps (no search step)
            await self.initialize_content(content)
            await self.step_1_format_content()
            await self.step_2_summarize_content()
            await self.step_3_categorize_content()
            await self.step_4_save_content()
            ...
            print(f"   💾 Memory ID: {final_state.get('memory_id', 'N/A')}")
            print(f"   📄 Summary Length: {final_state.get('summary_length', 0)} chars")
            
            print(f"\n⏱️  Step Timing Breakdown:")
            timing = final_state.get("timing", {})
            for step, duration in timing.items():
                if step.endswith('_time'):
                    step_name = step.replace('_time', '')
                    percentage = (duration / overall_time) * 100
                    print(f"   🔸 {step_name}: {duration:.2f}s ({percentage:.1f}%)")
            
            print(f"\n📁 Workflow state saved in: {self.json_file}")
...
async def test_json_workflow():
    """Test the JSON workflow orchestrator with placeholder content"""
    
    print("🧪 JSON WORKFLOW ORCHESTRATOR TEST")
    print("This demonstrates stateless tool architecture using JSON state")
    print("(Prototype for Redis hash implementation)")
    print()
    
    # PLACEHOLDER CONTENT - replace with your large content
    test_content = """
...
Thanks to Trevor Blackwell, Robert Morris, and Dan Giffin for reading drafts of this, and to Guido van Rossum, Jeremy Hylton, and the rest of the Python crew for inviting me to speak at PyCon.
    ...
    print("💡 TIP: Replace PLACEHOLDER_LARGE_CONTENT_HERE with your actual content!")
    print()
    
    # Create and run workflow
    orchestrator = JSONWorkflowOrchestrator()
    await orchestrator.run_full_workflow(test_content)

if __name__ == "__main__":
    print("🔬 Starting JSON Workflow Test...")
    asyncio.run(test_json_workflow()) 